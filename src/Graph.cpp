...
void Graph::CheckVertex(uint32_t v) const{
    if(!IsValidVertex(v)){
        std::string msg = "Vertex " + std::to_string(v) + " out of range";
        throw std::out_of_range(msg);
    }
}

Graph::Graph(uint32_t vertices) : vertices(vertices), edges(0){
    graphImp = new AdjacentListGraph(vertices);
}

Graph::~Graph(){ delete graphImp; }
uint32_t Graph::V() const{ return vertices; }
uint32_t Graph::E() const{ return edges; }

void Graph::AddEdge(uint32_t v, uint32_t w){
...
const std::forward_list<uint32_t>& Graph::Adj(uint32_t v) const{
    CheckVertex(v);
